...
use crate::infrastructure::components::{enemy::Enemy, Health};
    ...
    for event in damage_events.read() {
        ...
        if let Ok(invincibility) = invincibility_query.get(event.target) {
            if invincibility.is_active() {
                info!(
                    "Entity {:?} is invincible, damage ignored",
                    event.target
                );
...
pub fn death_system(
    ...
    health_query: Query<(Entity, &Health, &Transform), Without<crate::infrastructure::components::enemy::DeathAnimation>>,
    ...
    for (entity, health, transform) in &health_query {
        if health.is_dead() {
            ...
            if is_enemy {
                ...
                // For non-enemy entities, despawn immediately
